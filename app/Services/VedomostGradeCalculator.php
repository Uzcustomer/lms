 ...
 * generateYnQaydnoma() bilan AYNAN bir xil mantiq. Maqsad: AI tekshiruvga
 * tayyor (tizim qaydnomada chiqaradigan) qiymatlarni berish, AI esa faqat
 * skanerni o'qib solishtiradi (o'zi hisoblamaydi).
 *
 * Og'irliklar (JB/MT/ON/OSKI/Test) qaydnoma shablonining 19-qatoridan o'qiladi —
 * generator bilan bir xil manba.
 */
class VedomostGradeCalculator
{
    private const TEMPLATE = 'templates/yn_qaydnoma (1).xlsx';

    /** Og'irliklar keshi (bir so'rov davomida bir marta o'qiladi). */
    private static ?array $weights = null;

    /**
     * Shablondagi og'irliklar: ['jn'=>50,'mt'=>20,'on'=>0,'oski'=>0,'test'=>30].
     * O'qib bo'lmasa — xavfsiz default (eng keng tarqalgan sxema).
     */
    public function weights(): array
    {
        ...
        $default = ['jn' => 50, 'mt' => 20, 'on' => 0, 'oski' => 0, 'test' => 30];
        ...
        try {
            ...
            if (is_file($path)) {
                ...
                self::$weights = [
                    'jn'   => (int) ($sheet->getCell('D19')->getValue() ?? $default['jn']),
                    'mt'   => (int) ($sheet->getCell('G19')->getValue() ?? $default['mt']),
                    'on'   => (int) ($sheet->getCell('J19')->getValue() ?? $default['on']),
                    'oski' => (int) ($sheet->getCell('P19')->getValue() ?? $default['oski']),
                    'test' => (int) ($sheet->getCell('S19')->getValue() ?? $default['test']),
                ];
            ...
            // shablon o'qilmasa default'ga tushamiz
        }
     ...
     * @return array{jb_pct:?int,mt_pct:?int,on_pct:?int,oski_pct:?int,test_pct:?int,
     *               jb_ball:int|float,mt_ball:int|float,on_ball:int|float,
     *               oski_ball:int|float,test_ball:int|float,jbmton_ball:int|float,
     *               ozlashtirish:?int,ects:string,baho:string}
     */
    public function compute($jn, $mt, $on, $oski, $test, ?string $levelCode, float $davomatPct = 0.0): array
    {
        $w = $this->weights();
        $wJn = $w['jn'];
        $wMt = $w['mt'];
