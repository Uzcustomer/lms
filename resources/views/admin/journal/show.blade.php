<x-app-layout>
    {{-- ===== DEBUG PANEL ===== --}}
    @if(isset($debugInfo))
    <div id="debug-panel" style="background:#1e293b;color:#e2e8f0;padding:16px;margin:8px;border-radius:8px;font-family:monospace;font-size:12px;max-height:500px;overflow:auto;position:relative;z-index:9999;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
            <strong style="color:#fbbf24;font-size:14px;">JOURNAL DEBUG INFO</strong>
            <button onclick="document.getElementById('debug-panel').style.display='none'" style="background:#ef4444;color:white;border:none;padding:4px 12px;border-radius:4px;cursor:pointer;">Yopish</button>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
            <div>
                <div style="color:#60a5fa;font-weight:bold;margin-bottom:4px;">URL Parametrlari:</div>
            ...
            <div>
                ...
                <div>id: <span style="color:#34d399;">{{ $debugInfo['group']['id'] }}</span></div>
                <div>name: <span style="color:#34d399;">{{ $debugInfo['group']['name'] }}</span></div>
                <div>group_hemis_id: <span style="color:#fbbf24;">{{ $debugInfo['group']['group_hemis_id'] }}</span></div>
                <div>curriculum_hemis_id: <span style="color:#34d399;">{{ $debugInfo['group']['curriculum_hemis_id'] }}</span></div>
            </div>
            <div>
            ...
            <div>
                ...
                <div>curriculum->education_year_code: <span style="color:#f87171;">{{ $debugInfo['curriculum']['education_year_code'] ?? 'NULL' }}</span></div>
                <div>schedules dan topilgan: <span style="color:#f87171;">{{ $debugInfo['schedule_education_year_from_db'] ?? 'NULL' }}</span></div>
                <div>FINAL education_year_code: <span style="color:#fbbf24;font-weight:bold;">{{ $debugInfo['education_year_code_final'] ?? 'NULL' }}</span></div>
            </div>
            <div>
                <div style="color:#60a5fa;font-weight:bold;margin-bottom:4px;">Jadval (schedules) — group_hemis_id + subject_id + semester_code:</div>
                <div>Jami (filtrSIZ): <span style="color:#fbbf24;font-size:16px;font-weight:bold;">{{ $debugInfo['schedules_total'] }}</span></div>
                <div>lesson_date mavjud: <span style="color:#fbbf24;font-size:16px;font-weight:bold;">{{ $debugInfo['schedules_with_date'] }}</span></div>
                <div style="margin-top:4px;">education_year_code filtr BILAN:</div>
                <div>JB (amaliyot): <span style="color:#34d399;">{{ $debugInfo['jb_schedule_count'] ?? '?' }}</span></div>
                <div>MT: <span style="color:#34d399;">{{ $debugInfo['mt_schedule_count'] ?? '?' }}</span></div>
                <div>Ma'ruza: <span style="color:#34d399;">{{ $debugInfo['lecture_schedule_count'] ?? '?' }}</span></div>
            </div>
            <div>
            ...
            <div>
                <div style="color:#60a5fa;font-weight:bold;margin-bottom:4px;">Talabalar va baholar:</div>
                <div>Talabalar soni: <span style="color:#34d399;">{{ count($debugInfo['student_hemis_ids']) }}</span></div>
                <div>Student hemis_ids: <span style="color:#94a3b8;font-size:11px;">{{ implode(', ', array_slice($debugInfo['student_hemis_ids'], 0, 10)) }}{{ count($debugInfo['student_hemis_ids']) > 10 ? '...' : '' }}</span></div>
                <div>student_grades soni (filtrSIZ): <span style="color:#fbbf24;font-size:16px;font-weight:bold;">{{ $debugInfo['student_grades_count'] }}</span></div>
            </div>
            <div>
                <div style="color:#60a5fa;font-weight:bold;margin-bottom:4px;">Semester info:</div>
                @if($debugInfo['semester'])
                    <div>id: {{ $debugInfo['semester']['id'] }}, code: {{ $debugInfo['semester']['code'] }}, name: {{ $debugInfo['semester']['name'] }}</div>
                @else
                    <div style="color:#f87171;">Semester TOPILMADI!</div>
                @endif
            </div>
        </div>
        @if(count($debugInfo['schedules_sample']) > 0)
        <div style="margin-top:12px;">
            <div style="color:#60a5fa;font-weight:bold;margin-bottom:4px;">Schedule namunalari (5 ta):</div>
            <table style="width:100%;border-collapse:collapse;font-size:11px;">
                <tr style="color:#94a3b8;">
                    <th style="text-align:left;padding:2px 6px;">id</th>
                    <th style="text-align:left;padding:2px 6px;">group_id</th>
                    <th style="text-align:left;padding:2px 6px;">subject_id</th>
                    <th style="text-align:left;padding:2px 6px;">semester_code</th>
                    <th style="text-align:left;padding:2px 6px;">education_year_code</th>
                    <th style="text-align:left;padding:2px 6px;">lesson_date</th>
                    <th style="text-align:left;padding:2px 6px;">training_type_code</th>
                    <th style="text-align:left;padding:2px 6px;">training_type_name</th>
                </tr>
                @foreach($debugInfo['schedules_sample'] as $row)
                <tr style="border-top:1px solid #334155;">
                    <td style="padding:2px 6px;">{{ $row->id }}</td>
                    <td style="padding:2px 6px;">{{ $row->group_id }}</td>
                    <td style="padding:2px 6px;">{{ $row->subject_id }}</td>
                    <td style="padding:2px 6px;">{{ $row->semester_code }}</td>
                    <td style="padding:2px 6px;color:#fbbf24;">{{ $row->education_year_code ?? 'NULL' }}</td>
                    <td style="padding:2px 6px;">{{ $row->lesson_date ?? 'NULL' }}</td>
                    <td style="padding:2px 6px;">{{ $row->training_type_code }}</td>
                    <td style="padding:2px 6px;">{{ $row->training_type_name }}</td>
                </tr>
                @endforeach
            </table>
        </div>
        @endif
        @if(count($debugInfo['student_grades_sample']) > 0)
        <div style="margin-top:12px;">
            <div style="color:#60a5fa;font-weight:bold;margin-bottom:4px;">Baho namunalari (5 ta):</div>
            <table style="width:100%;border-collapse:collapse;font-size:11px;">
                <tr style="color:#94a3b8;">
                    <th style="text-align:left;padding:2px 6px;">id</th>
                    <th style="text-align:left;padding:2px 6px;">student_hemis_id</th>
                    <th style="text-align:left;padding:2px 6px;">subject_id</th>
                    <th style="text-align:left;padding:2px 6px;">semester_code</th>
                    <th style="text-align:left;padding:2px 6px;">education_year_code</th>
                    <th style="text-align:left;padding:2px 6px;">lesson_date</th>
                    <th style="text-align:left;padding:2px 6px;">training_type_code</th>
                    <th style="text-align:left;padding:2px 6px;">grade</th>
                    <th style="text-align:left;padding:2px 6px;">status</th>
                </tr>
                @foreach($debugInfo['student_grades_sample'] as $row)
                <tr style="border-top:1px solid #334155;">
                    <td style="padding:2px 6px;">{{ $row->id }}</td>
                    <td style="padding:2px 6px;">{{ $row->student_hemis_id }}</td>
                    <td style="padding:2px 6px;">{{ $row->subject_id }}</td>
                    <td style="padding:2px 6px;">{{ $row->semester_code }}</td>
                    <td style="padding:2px 6px;color:#fbbf24;">{{ $row->education_year_code ?? 'NULL' }}</td>
                    <td style="padding:2px 6px;">{{ $row->lesson_date ?? 'NULL' }}</td>
                    <td style="padding:2px 6px;">{{ $row->training_type_code }}</td>
                    <td style="padding:2px 6px;">{{ $row->grade }}</td>
                    <td style="padding:2px 6px;">{{ $row->status }}</td>
                </tr>
                @endforeach
            </table>
        </div>
            ...
            <div id="content-amaliyot" class="tab-content">
                                    ...
                                    @foreach ($students as $index => $student)
                                        ...
                                        <tr>
                                            ...
                                            @php
                                                ...
                                                $consentStatus = $consent ? $consent->status : null;
                                            @endphp
                                            <td class="px-1 py-1 text-center">
                                                @if($consentStatus === 'approved')
                                                    <span class="inline-block w-4 h-4 rounded-full bg-green-500" title="YN topshirishga tayyor"></span>
                                                @elseif($consentStatus === 'rejected')
                                                    <span class="inline-block w-4 h-4 rounded-full bg-red-500" title="Rozi emas"></span>
                                                @else
                                                    <span class="inline-block w-4 h-4 rounded-full bg-yellow-400" title="Hali bosmagan"></span>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
                ...
                <div class="mt-4 p-4 bg-gray-50 border rounded-lg">
                    <div class="flex items-center justify-between">
                        <div>
                            <h4 class="font-semibold text-gray-800">Yakuniy nazorat (YN) ga yuborish</h4>
                            <div class="flex items-center space-x-4 mt-2 text-sm">
                                <span class="flex items-center"><span class="inline-block w-3 h-3 rounded-full bg-green-500 mr-1"></span> Tayyor</span>
                                <span class="flex items-center"><span class="inline-block w-3 h-3 rounded-full bg-yellow-400 mr-1"></span> Kutilmoqda</span>
                                <span class="flex items-center"><span class="inline-block w-3 h-3 rounded-full bg-red-500 mr-1"></span> Rozi emas</span>
                            </div>
                            @php
                                $totalStudents = count($students);
                                $approvedCount = ($ynConsents ?? collect())->where('status', 'approved')->count();
                                $rejectedCount = ($ynConsents ?? collect())->where('status', 'rejected')->count();
                                $pendingCount = $totalStudents - $approvedCount - $rejectedCount;
                            @endphp
                            <p class="text-sm text-gray-600 mt-2">
                                Jami: {{ $totalStudents }} | Tayyor: <span class="text-green-600 font-semibold">{{ $approvedCount }}</span> | Kutilmoqda: <span class="text-yellow-600 font-semibold">{{ $pendingCount }}</span> | Rozi emas: <span class="text-red-600 font-semibold">{{ $rejectedCount }}</span>
                            </p>
                        </div>
                        <div>
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            <h4 class="font-semibold text-amber-800 flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 mr-2" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" /></svg>
                                Sababli baholar (tasdiqlangan hujjatlar)
                            </h4>
                            <p class="text-sm text-amber-700 mt-1">
                                {{ $approvedExcuses->count() }} ta talabaning sababli hujjati tasdiqlangan.
                                NB katakchalarni bosib baho kiriting, keyin "YN ga yuborish" tugmasini bosing.
                            </p>
                            <div class="mt-2 flex flex-wrap gap-2">
                                @foreach($approvedExcuses as $excuse)
                                    @php
                                        $alreadySaved = isset($excuseGradeSnapshots[$excuse->student_hemis_id]);
                                    @endphp
                                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $alreadySaved ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800' }}">
                                        {{ $excuse->student_full_name }}
                                        @if($alreadySaved) &#10003; @endif
                                    </span>
                                @endforeach
                            </div>
                        </div>
                        <div>
                            @if($excuseGradeSnapshots->isNotEmpty() && $excuseGradeSnapshots->count() >= $approvedExcuses->count())
                                <div class="bg-green-100 text-green-800 px-4 py-2 rounded-lg font-medium text-sm">
                                    Sababli baholar YN ga yuborilgan
                                </div>
                            @elseif($canSubmitYn ?? false)
                                <button type="button" id="btn-submit-excuse-yn"
                                    class="px-6 py-3 bg-amber-600 text-white font-semibold rounded-lg hover:bg-amber-700 transition shadow-sm"
                                    onclick="submitExcuseToYn()">
                                    Sababli baholarni YN ga yuborish
                                </button>
                            @endif
                        </div>
                    </div>
    ...
    <script>
        ...
        function openExcuseModal(studentHemisId, studentName, gradeId, excuseId) {
            ...
            // Data atributlarga yozish
            const modal = document.getElementById('excuse-modal');
            modal.dataset.studentHemisId = studentHemisId;
            ...
            document.getElementById('excuse-modal-grade-input').focus();
        }
        ...
        function saveExcuseGrade() {
            const modal = document.getElementById('excuse-modal');
            const grade = parseFloat(document.getElementById('excuse-modal-grade-input').value);
            const comment = document.getElementById('excuse-modal-comment').value.trim();
            ...
            const btn = document.getElementById('excuse-modal-save-btn');
            btn.disabled = true;
            btn.textContent = 'Saqlanmoqda...';

            fetch('{{ route("admin.journal.save-excuse-grade") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                ...
                body: JSON.stringify({
                    student_hemis_id: modal.dataset.studentHemisId,
                    subject_id: '{{ $subjectId }}',
                    semester_code: '{{ $semesterCode }}',
                    group_hemis_id: '{{ $group->group_hemis_id }}',
                    grade_id: parseInt(modal.dataset.gradeId),
                    grade: grade,
                    comment: comment,
                    absence_excuse_id: parseInt(modal.dataset.excuseId),
                })
            })
            .then(r => r.json().then(data => ({ok: r.ok, data})))
            .then(({ok, data}) => {
                if (ok && data.success) {
                    ...
                    // Muvaffaqiyat xabari
                    const notif = document.createElement('div');
                    notif.style.cssText = 'position:fixed; top:20px; left:50%; transform:translateX(-50%); z-index:99999; background:#f59e0b; color:#fff; padding:12px 24px; border-radius:10px; box-shadow:0 4px 16px rgba(0,0,0,0.2); font-size:14px; font-weight:600;';
                    notif.textContent = 'Sababli baho saqlandi: ' + data.grade;
                    document.body.appendChild(notif);
                    setTimeout(() => { notif.remove(); location.reload(); }, 1500);
                } else {
                    alert(data.message || 'Xatolik yuz berdi');
                    btn.disabled = false;
                    btn.textContent = 'Saqlash';
        ...
        function submitExcuseToYn() {
            const warn = confirm(
                'Sababli talabalarning yangilangan baholari YN ga yuboriladi.\n\n' +
                'Davom etasizmi?'
            );
            if (!warn) return;

            const btn = document.getElementById('btn-submit-excuse-yn');
            btn.disabled = true;
            btn.textContent = 'Yuborilmoqda...';
            btn.style.opacity = '0.6';

            fetch('{{ route("admin.journal.submit-excuse-to-yn") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
    ...
    <div id="excuse-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" onclick="if(event.target===this) closeExcuseModal()">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
            <div class="bg-amber-500 px-6 py-4 text-white">
                <h3 class="text-lg font-bold">Sababli baho kiritish</h3>
                <p class="text-amber-100 text-sm">Tasdiqlangan sababli hujjat asosida</p>
            </div>
            <div class="px-6 py-4 space-y-4">
                {{-- Talaba ma'lumotlari --}}
                <div class="bg-gray-50 rounded-lg p-3">
                    <div class="text-sm text-gray-600">Talaba:</div>
                    <div class="font-semibold text-gray-900" id="excuse-modal-student-name"></div>
                    <div class="text-xs text-gray-500">HEMIS ID: <span id="excuse-modal-hemis-id"></span></div>
                </div>
                {{-- Sababli hujjat ma'lumotlari --}}
                <div class="bg-amber-50 rounded-lg p-3 border border-amber-100">
                    <div class="text-sm font-medium text-amber-800 mb-1">Sababli hujjat:</div>
                    <div class="text-sm text-amber-700">
                        <div>Sabab: <span class="font-medium" id="excuse-modal-reason"></span></div>
                        <div>Muddat: <span class="font-medium" id="excuse-modal-dates"></span></div>
                        <div>Hujjat №: <span class="font-medium" id="excuse-modal-doc"></span></div>
                    </div>
                </div>
                {{-- Baho kiritish --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Baho (0-100)</label>
                    <input type="number" id="excuse-modal-grade-input" min="0" max="100" step="1"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 text-lg font-semibold text-center"
                           placeholder="Bahoni kiriting"
                           onkeydown="if(event.key==='Enter') saveExcuseGrade(); if(event.key==='Escape') closeExcuseModal();">
                </div>
                {{-- Izoh --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Izoh (ixtiyoriy)</label>
                    <textarea id="excuse-modal-comment" rows="2"
            ...
            <div class="px-6 py-4 bg-gray-50 flex justify-end space-x-3">
                <button type="button" onclick="closeExcuseModal()" class="px-4 py-2 text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                    Bekor qilish
                </button>
                <button type="button" id="excuse-modal-save-btn" onclick="saveExcuseGrade()"
