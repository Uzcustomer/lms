...
test('birinchi kun band bo\'lsa ma\'ruza va amaliy paketi boshqa kunga to\'liq ko\'chadi', function () {
    ...
    $standIn = autoFlowCard($board, [
        'training_type' => 'practice',
        'group_name' => '1K-01a',
        'subject_name' => 'Odam anatomiyasi',
        'weeks' => 5,
    ], [2, 4, 6, 8, 10]);
    $weekly = autoFlowCard($board, [
        'training_type' => 'practice',
        'group_name' => '1K-01a',
        'subject_name' => 'Odam anatomiyasi',
    ]);
    ...
    expect($lecture->fresh()->day)->toBe(2)
        ->and($lecture->fresh()->pair)->toBe(1)
        ->and($standIn->fresh()->day)->toBe(2)
        ->and($standIn->fresh()->pair)->toBe(1)
        ->and($weekly->fresh()->day)->toBe(2)
        ->and($weekly->fresh()->pair)->toBe(3);
});

test('faqat ma\'ruza rejimi amaliy paketiga bog\'lanmasdan ishlaydi', function () {
    $board = TimetableBoard::create([
        'name' => 'Test',
        'days' => 1,
        'pairs_per_day' => 2,
    ...
    (new TimetableController)->autoPlace(
        Request::create('/', 'POST', ['training_type' => 'lecture']),
        $board
    );
