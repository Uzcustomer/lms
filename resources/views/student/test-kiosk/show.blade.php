...
<head>
    ...
    <style>
        ...
        .card{background:#fff;border:1px solid #dbe4ef;border-radius:24px;box-shadow:0 10px 28px rgba(15,23,42,.06);overflow:hidden}
        .head{padding:18px 20px;background:linear-gradient(135deg,#eff6ff,#dbeafe);border-bottom:1px solid #dbe4ef}
        ...
        .btn{display:inline-flex;align-items:center;justify-content:center;border-radius:12px;padding:12px 18px;font-size:14px;font-weight:800;text-decoration:none;border:none;cursor:pointer}
...
<div class="wrap">
    ...
    <div class="card">
        <div class="head">
            ...
            <h1 style="margin:14px 0 0;font-size:32px;line-height:1.1;">{{ $lesson->topic_title ?: ($lesson->topic_order . '-mavzu') }}</h1>
            <p style="margin:8px 0 0;color:#475569;">{{ $lessonTest->title }}</p>
        </div>
        <div class="body">
            <div style="margin-bottom:16px;display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                <div style="font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#64748b;margin-right:6px;">Test tili</div>
            ...
            <div class="stat-grid">
                <div class="stat"><div style="font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#64748b;">Savollar</div><div style="margin-top:6px;font-size:22px;font-weight:800;">{{ $questions->count() }}</div></div>
                <div class="stat"><div style="font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#64748b;">Davomiylik</div><div style="margin-top:6px;font-size:22px;font-weight:800;">{{ $lessonTest->duration_minutes }} min</div></div>
                <div class="stat"><div style="font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#64748b;">O'tish foizi</div><div style="margin-top:6px;font-size:22px;font-weight:800;">{{ $passPercent }}%</div></div>
                <div class="stat"><div style="font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#64748b;">{{ $submitted ? 'Natija' : 'Qolgan vaqt' }}</div><div style="margin-top:6px;font-size:22px;font-weight:800;" id="test-timer">{{ $submitted ? ($attempt->percent . '%') : gmdate('H:i:s', $remainingSeconds) }}</div></div>
            </div>
        </div>
    </div>
    ...
    @if($submitted)
        <div class="card" style="margin-top:16px;">
            <div class="body">
                <div class="stat-grid">
                    <div class="stat"><div style="font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#64748b;">To'plagan ball</div><div style="margin-top:6px;font-size:22px;font-weight:800;">{{ rtrim(rtrim((string) $attempt->score, '0'), '.') }} / {{ $attempt->total_points }}</div></div>
                    <div class="stat"><div style="font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#64748b;">Foiz</div><div style="margin-top:6px;font-size:22px;font-weight:800;">{{ $attempt->percent }}%</div></div>
                    <div class="stat"><div style="font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#64748b;">Javoblar</div><div style="margin-top:6px;font-size:22px;font-weight:800;">{{ $attempt->answers_count }}</div></div>
                    <div class="stat"><div style="font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#64748b;">Holat</div><div style="margin-top:10px;font-size:18px;font-weight:800;color:{{ $attempt->is_passed ? '#059669' : '#dc2626' }};">{{ $attempt->is_passed ? 'Muvaffaqiyatli' : 'Qayta ishlash kerak' }}</div></div>
                </div>
            </div>
        </div>
    @endif
            ...
            @foreach($questions as $index => $question)
                <div class="q">
                    <div class="qh" style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;">
                        <div style="font-weight:800;">{{ $index + 1 }}. {{ $question->promptFor($language) }}</div>
                        ...
                        @if($question->type === 'single_choice')
                            @foreach($question->options as $optionIndex => $option)
                                <label class="opt">
                                    ...
                                    <div style="flex:1;">
                                        <div style="font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#94a3b8;">{{ chr(65 + $optionIndex) }})</div>
                                        <div style="margin-top:6px;font-size:15px;font-weight:600;">{{ $option->textFor($language) }}</div>
                                    </div>
                                </label>
                            @endforeach
                        @else
