...
<div class="min-h-screen bg-gray-100 dark:bg-gray-900">
    ...
    <div x-data="{ boshqalarOpen: false }" class="sm:hidden" style="position:fixed !important;bottom:0 !important;left:0 !important;right:0 !important;z-index:9999 !important;">
        ...
        <div class="flex items-end justify-around bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 px-2 pt-2" style="padding-bottom:max(0.5rem, env(safe-area-inset-bottom));">
            <!-- 1. Fanlar -->
            <a href="{{ route('student.subjects') }}" class="flex flex-col items-center justify-center w-16 {{ $activeTab === 'fanlar' ? '-mt-3' : 'gap-0.5' }}">
                @if($activeTab === 'fanlar')
                    ...
                    <span class="text-[10px] font-medium leading-tight mt-0.5 text-indigo-600 dark:text-indigo-400">Fanlar</span>
                @else
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" />
                    </svg>
                    <span class="text-[10px] font-medium leading-tight text-gray-500 dark:text-gray-400">Fanlar</span>
                @endif
            </a>

            <!-- 2. Dars jadvali -->
            <a href="{{ route('student.schedule') }}" class="flex flex-col items-center justify-center w-16 {{ $activeTab === 'jadval' ? '-mt-3' : 'gap-0.5' }}">
                @if($activeTab === 'jadval')
                    <div class="w-12 h-12 rounded-full flex items-center justify-center bg-indigo-600 text-white shadow-lg shadow-indigo-300 dark:shadow-indigo-900">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                    ...
                    <span class="text-[10px] font-medium leading-tight mt-0.5 text-indigo-600 dark:text-indigo-400">Dars jadvali</span>
                @else
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" />
                    </svg>
                    <span class="text-[10px] font-medium leading-tight text-gray-500 dark:text-gray-400">Dars jadvali</span>
                @endif
            </a>

            <!-- 3. Asosiy -->
            <a href="{{ route('student.dashboard') }}" class="flex flex-col items-center justify-center w-16 {{ $activeTab === 'asosiy' ? '-mt-3' : 'gap-0.5' }}">
                @if($activeTab === 'asosiy')
                    <div class="w-12 h-12 rounded-full flex items-center justify-center bg-indigo-600 text-white shadow-lg shadow-indigo-300 dark:shadow-indigo-900">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                    ...
                    <span class="text-[10px] font-medium leading-tight mt-0.5 text-indigo-600 dark:text-indigo-400">Asosiy</span>
                @else
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z" />
                    </svg>
                    <span class="text-[10px] font-medium leading-tight text-gray-500 dark:text-gray-400">Asosiy</span>
                @endif
            </a>

            <!-- 4. MT (Mustaqil ta'lim) -->
            <a href="{{ route('student.independents') }}" class="flex flex-col items-center justify-center w-16 {{ $activeTab === 'mt' ? '-mt-3' : 'gap-0.5' }}">
                @if($activeTab === 'mt')
                    <div class="w-12 h-12 rounded-full flex items-center justify-center bg-indigo-600 text-white shadow-lg shadow-indigo-300 dark:shadow-indigo-900">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                    ...
                    <span class="text-[10px] font-medium leading-tight mt-0.5 text-indigo-600 dark:text-indigo-400">MT</span>
                @else
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M4.26 10.147a60.438 60.438 0 00-.491 6.347A48.627 48.627 0 0112 20.904a48.627 48.627 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.636 50.636 0 00-2.658-.813A59.906 59.906 0 0112 3.493a59.903 59.903 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0112 13.489a50.702 50.702 0 017.74-3.342M6.75 15a.75.75 0 100-1.5.75.75 0 000 1.5zm0 0v-3.675A55.378 55.378 0 0112 8.443m-7.007 11.55A5.981 5.981 0 006.75 15.75v-1.5" />
                    </svg>
                    <span class="text-[10px] font-medium leading-tight text-gray-500 dark:text-gray-400">MT</span>
                @endif
            </a>
