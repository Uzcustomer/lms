<x-guest-layout>
    ...
    <div class="login-card desktop-split" style="display:grid; grid-template-columns:1fr 1fr; width:100%; max-width:880px;">
        ...
        <div class="split-left">
            ...
            <form method="POST" action="{{ route('student.login.post') }}" id="student-form">
                @csrf
                ...
                @if($errors->has('login'))
                    <div class="form-error" style="margin-top:-0.75rem; margin-bottom:0.75rem;">{{ $errors->first('login') }}</div>
                @endif
        ...
        <div class="split-right">
            ...
            <form method="POST" action="{{ route('teacher.login.post') }}" id="teacher-form" style="width:100%; max-width:320px; position:relative; z-index:2;">
                @csrf
                <div class="login-input-group">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0" /></svg>
    ...
    <div class="login-card mobile-tabs" style="width:100%; max-width:420px;">
        <div style="display:flex; border-bottom:2px solid #e2e8f0;" id="mobile-tab-bar">
            <button onclick="switchMobileTab('student')" id="tab-student" style="flex:1; padding:14px 0; font-size:14px; font-weight:600; background:none; border:none; cursor:pointer; border-bottom:3px solid #0d9488; color:#0d9488; margin-bottom:-2px; transition:all 0.2s;">
                Talaba
            </button>
            <button onclick="switchMobileTab('teacher')" id="tab-teacher" style="flex:1; padding:14px 0; font-size:14px; font-weight:500; background:none; border:none; cursor:pointer; border-bottom:3px solid transparent; color:#64748b; margin-bottom:-2px; transition:all 0.2s;">
                Xodim
            </button>
        </div>

        {{-- Talaba tab --}}
        <div id="mobile-student" class="mobile-tab-content">
            ...
            <form method="POST" action="{{ route('student.login.post') }}">
                ...
                <div class="remember-check">
                    <input id="m_remember_student" type="checkbox" name="remember">
                    <label for="m_remember_student">Eslab qolish</label>
                </div>
        ...
        {{-- Xodim tab --}}
        <div id="mobile-teacher" class="mobile-tab-content" style="display:none;">
            <div style="margin-bottom:1.5rem;">
            ...
            <form method="POST" action="{{ route('teacher.login.post') }}">
                ...
                <div class="remember-check">
                    <input id="m_remember_teacher" type="checkbox" name="remember">
                    <label for="m_remember_teacher">Eslab qolish</label>
                </div>
