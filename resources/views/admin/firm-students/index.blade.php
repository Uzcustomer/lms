    ...
    <div class="py-4">
        <div class="max-w-full mx-auto sm:px-4 lg:px-6">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                {{-- Filtrlar --}}
                <form method="GET" action="{{ route('admin.firm-students.index') }}">
                ...
                {{-- Statistika --}}
                <div style="padding:10px 20px;background:#f8fafc;border-bottom:1px solid #e2e8f0;display:flex;align-items:center;gap:8px;">
                    ...
                    <table class="student-table">
                        ...
                        <tbody>
                            @forelse($students as $i => $student)
                                ...
                                @endphp
                                <tr class="{{ $isUrgent ? 'int-row-urgent' : '' }}" onclick="window.location='{{ route('admin.firm-students.show', $student) }}'" style="cursor:pointer;">
                                    <td style="text-align:center;color:#94a3b8;font-size:12px;">{{ $students->firstItem() + $i }}</td>
                                    <td>
                                        <a href="{{ route('admin.firm-students.show', $student) }}" class="student-name-link">{{ $student->full_name }}</a>
                                    </td>
                                    ...
                                    <td style="text-align:center;" onclick="event.stopPropagation();">
                                        @if($vp === 'passport_accepted' || $vp === 'registering')
                                            <span style="font-size:10px;font-weight:700;padding:2px 8px;border-radius:4px;background:#fef3c7;color:#92400e;">Viza yangilanmoqda</span>
                                        @elseif($rp === 'passport_accepted' || $rp === 'registering')
                                            <span style="font-size:10px;font-weight:700;padding:2px 8px;border-radius:4px;background:#fef3c7;color:#92400e;">Reg. qilinmoqda</span>
                                        @elseif(!$visa->passport_handed_over && in_array($rp, ['none','done']) && in_array($vp, ['none','done']))
                                        ...
                                        @elseif($rp === 'done' || $vp === 'done')
                                            <span style="font-size:10px;font-weight:600;padding:2px 8px;border-radius:4px;background:#dcfce7;color:#166534;">Tugallandi</span>
                                        @else
                                            <span class="int-empty">—</span>
                                        @endif
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" style="text-align:center;padding:40px 20px;color:#94a3b8;font-size:14px;">Talabalar topilmadi</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div style="padding:12px 20px;border-top:1px solid #e2e8f0;background:#f8fafc;">
                    ...
                    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <p class="text-sm text-gray-700">
                            {!! __('Showing') !!} <span class="font-medium">{{ $students->firstItem() }}</span>
                            {!! __('to') !!} <span class="font-medium">{{ $students->lastItem() }}</span>
                            {!! __('of') !!} <span class="font-medium">{{ $students->total() }}</span> {!! __('results') !!}
                        </p>
