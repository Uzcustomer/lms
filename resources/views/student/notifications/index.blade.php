                    ...
                    @else
                        ...
                        @php
                            ...
                            if ($notification->type === 'sms') {
                                ...
                                $excuseStatus = $notification->data['status'] ?? '';
                                $iconBg = $excuseStatus === 'approved' ? 'bg-green-100' : 'bg-red-100';
                                $iconColor = $excuseStatus === 'approved' ? 'text-green-600' : 'text-red-600';
                            }
                        @endphp
                        <div class="bg-white rounded-xl border {{ $notification->isRead() ? 'border-gray-200' : 'border-indigo-200 bg-indigo-50/30' }} p-4 transition">
                            <div class="flex items-start gap-3">
                                <div class="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 {{ $iconBg }}">
                                    @if($notification->type === 'absence_excuse')
                                        <svg class="w-4.5 h-4.5 {{ $iconColor }}" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
