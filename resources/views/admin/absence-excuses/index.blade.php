<x-app-layout>
    <div class="p-4 sm:ml-64">
        <div class="mt-14">
            ...
            <div id="reviewerStatsModal" class="hidden fixed inset-0 z-50 overflow-y-auto" x-data="{ openReviewer: null, filterStatus: null }">
                <div class="flex items-center justify-center min-h-screen px-4 py-8">
                    ...
                    <div class="relative bg-white rounded-lg shadow-xl max-w-4xl w-full z-10 max-h-[90vh] flex flex-col">
                                    ...
                                    @foreach($reviewerStats as $i => $reviewer)
                                        <div class="border rounded-lg overflow-hidden">
                                            ...
                                            <div class="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-gray-50 transition"
                                                 @click="openReviewer === {{ $reviewer->reviewed_by }} ? (openReviewer = null, filterStatus = null) : (openReviewer = {{ $reviewer->reviewed_by }}, filterStatus = null)">
                                                <div class="flex items-center gap-3">
                                                    <span class="text-sm text-gray-400 w-5">{{ $i + 1 }}</span>
                                                    <span class="text-sm font-semibold text-gray-900">{{ $reviewer->reviewed_by_name }}</span>
                                                </div>
                                                <div class="flex items-center gap-2">
                                                    <span class="px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-800">{{ $reviewer->approved_count }} tasdiqlangan</span>
                                                    <span class="px-2 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-800">{{ $reviewer->rejected_count }} rad etilgan</span>
                                                    <span class="px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-800">{{ $reviewer->total_count }} jami</span>
                                                    <svg class="w-4 h-4 text-gray-400 transition-transform" :class="openReviewer === {{ $reviewer->reviewed_by }} ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            {{-- Reviewer arizalari --}}
                                            <div x-show="openReviewer === {{ $reviewer->reviewed_by }}" x-transition>
                                                <div class="border-t bg-gray-50 px-4 py-2">
                                                    <div class="flex gap-2 mb-2">
                                                        <button @click="filterStatus = null"
                                                                :class="filterStatus === null ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'"
                                                                class="px-2.5 py-1 text-xs rounded-full font-medium transition">
                                                            Barchasi ({{ $reviewer->total_count }})
                                                        </button>
                                                        <button @click="filterStatus = 'approved'"
                                                                ...
                                                                class="px-2.5 py-1 text-xs rounded-full font-medium transition">
                                                            Tasdiqlangan ({{ $reviewer->approved_count }})
                                                        </button>
                                                        <button @click="filterStatus = 'rejected'"
                                                                ...
                                                                class="px-2.5 py-1 text-xs rounded-full font-medium transition">
                                                            Rad etilgan ({{ $reviewer->rejected_count }})
                                                        </button>
                                                    </div>
                                                </div>
                                                    ...
                                                    <table class="min-w-full divide-y divide-gray-200 text-xs">
                                                        ...
                                                        <tbody class="divide-y divide-gray-100">
                                                            @foreach(($reviewerExcuses[$reviewer->reviewed_by] ?? collect()) as $exc)
                                                                <tr x-show="filterStatus === null || filterStatus === '{{ $exc->status }}'"
