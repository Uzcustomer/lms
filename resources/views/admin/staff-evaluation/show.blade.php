<x-app-layout>
            ...
            <div class="flex-shrink-0 bg-white p-3 border rounded-lg">
                <div class="relative inline-block">
                    ...
                    <div class="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <div class="bg-white rounded-full" style="padding:5px;">
                            <img src="{{ asset('logo.png') }}" alt="Logo" class="rounded-full" style="width:56px;height:56px;">
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white shadow rounded-lg p-4 mb-6">
        <div class="flex flex-wrap items-center gap-2">
            ...
            <a href="{{ route('admin.staff-evaluation.export-excel', array_merge(['teacher' => $teacher], request()->only('rating'))) }}"
               class="ml-auto px-4 py-1.5 bg-green-700 text-white rounded-lg text-sm font-medium hover:bg-green-800 inline-flex items-center gap-2">
    ...
    <div class="bg-white shadow rounded-lg p-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
            ...
            @endphp
            <div class="rounded-lg border p-4 flex gap-3 {{ $cardBg }}">
                <div class="flex-shrink-0 w-7 h-7 rounded-full bg-white border flex items-center justify-center text-xs font-bold text-gray-500">
                    {{ $orderNum }}
                </div>
                <div class="flex-1 min-w-0">
                    ...
                    @if($eval->comment)
                        <p class="text-gray-700 mt-1 text-sm">{{ $eval->comment }}</p>
                    @endif
                    @if($eval->student)
