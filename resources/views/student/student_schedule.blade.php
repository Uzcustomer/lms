    ...
    <div x-data="{
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm">
            <div class="px-4 py-1.5">
                    ...
                    <div class="flex items-center gap-2">
                        @if($prevWeekId)
                            <a href="{{ route('student.schedule', ['semester_id' => $selectedSemester['id'] ?? '', 'week_id' => $prevWeekId]) }}"
                               class="w-6 h-6 flex items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500">
                ...
                {{-- Day selector row --}}
                <div class="flex gap-1 justify-center mt-[10px]">
                    @foreach($weekDates as $wd)
                        @php $color = $dayColors[$wd['day_en']]; @endphp
                        <button @click="view = 'day'; selectedDay = '{{ $wd['day_en'] }}'"
                                :class="view === 'day' && selectedDay === '{{ $wd['day_en'] }}' ? 'bg-blue-500 text-white shadow-md scale-105' : '{{ $color['light'] }} {{ $color['text'] }}'"
                        ...
                        <div class="flex flex-col gap-[5px]">
                            @foreach($groupedSchedule[$dayEn] as $lesson)
                                <div class="flex gap-2 items-start">
                                    {{-- Vaqt --}}
                                    <div class="w-10 flex-shrink-0 text-right pt-2">
                                        <div class="text-[11px] font-bold text-gray-700 dark:text-gray-300 leading-tight">{{ \Carbon\Carbon::parse($lesson['lessonPair']['start_time'])->format('H:i') }}</div>
                                        <div class="text-[9px] text-gray-400 dark:text-gray-500 leading-tight">{{ \Carbon\Carbon::parse($lesson['lessonPair']['end_time'])->format('H:i') }}</div>
                                    </div>
                                    {{-- Dars kartochkasi --}}
                                    <div class="flex-1 {{ $color['bg'] }} border {{ $color['border'] }} rounded-[5px] px-3 py-2 relative overflow-hidden">
                                        <div class="absolute left-0 top-0 bottom-0 w-1 {{ $color['accent'] }} rounded-l-[5px]"></div>
                                        <div class="pl-1.5">
                        ...
                        <div class="flex flex-col gap-[5px]">
                            @foreach($groupedSchedule[$dayEn] as $lesson)
                                <div class="flex gap-2.5 items-start">
                                    {{-- Vaqt --}}
                                    <div class="w-10 flex-shrink-0 text-right pt-3">
                                        <div class="text-sm font-bold text-gray-700 dark:text-gray-300 leading-tight">{{ \Carbon\Carbon::parse($lesson['lessonPair']['start_time'])->format('H:i') }}</div>
                                        <div class="text-xs text-gray-400 dark:text-gray-500 leading-tight">{{ \Carbon\Carbon::parse($lesson['lessonPair']['end_time'])->format('H:i') }}</div>
                                    </div>
                                    {{-- Dars kartochkasi (kattaroq) --}}
                                    <div class="flex-1 {{ $color['bg'] }} border {{ $color['border'] }} rounded-[5px] px-4 py-3 relative overflow-hidden">
                                        <div class="absolute left-0 top-0 bottom-0 w-1.5 {{ $color['accent'] }} rounded-l-[5px]"></div>
                                        <div class="pl-2">
