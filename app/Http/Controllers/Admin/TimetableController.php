...
class TimetableController extends Controller
{
    ...
    public function autoPlace(Request $request, TimetableBoard $board)
    {
        ...
        foreach ($all as $card) {
            if ($card->training_type === 'lecture') {
                $lecturesByBase[$this->placementBaseKey($card)][] = $card;
            }
        }
        // Eski kartalarda oqim_label o'zgargan bo'lishi mumkin. Amaliy guruhni
        // ma'ruzaning group_names ro'yxati orqali bog'lash ancha ishonchli.
        $lectureMatchCache = [];
        $lectureForPractice = function ($card) use (&$lectureMatchCache, $lecturesByBase) {
            if ($card->training_type !== 'practice') {
                return null;
            }
            $id = (int) $card->id;
            if (!array_key_exists($id, $lectureMatchCache)) {
                $lectureMatchCache[$id] = $this->matchingLectureForPractice($card, $lecturesByBase);
            }
            return $lectureMatchCache[$id];
        };
        $packageKeyForCard = function ($card) use ($lectureForPractice): string {
            if ($card->training_type === 'lecture') {
                return $this->placementSubjectKey($card);
            }
            $lecture = $lectureForPractice($card);
            return $lecture
                ? $this->placementSubjectKey($lecture)
                : $this->placementSubjectKey($card);
        };

        // Fan paketlari navbat bilan yuradi: ma'ruza, shu ma'ruzaning amaliy
        // guruhlari, keyin boshqa fan. Shunda keyingi fan kerakli slotni oldindan
        // egallamaydi; bitta guruh sig'masa qolgan guruhlar joylashaveradi.
        $baseOrder = fn($card) => [
            (string) $card->specialty_name, (int) $card->course, (int) $card->weeks,
            -count($card->occupiedGroups()), -(int) $card->students,
            $this->normSubject((string) $card->subject_name),
            (string) $card->oqim_label, (int) $card->id,
        ];
        $orderedLectures = $all->filter(fn($card) => $card->training_type === 'lecture')
            ->sort(fn($a, $b) => $baseOrder($a) <=> $baseOrder($b));
        $packageRanks = [];
        foreach ($orderedLectures as $lecture) {
            $key = $this->placementSubjectKey($lecture);
            if (!isset($packageRanks[$key])) {
                $packageRanks[$key] = count($packageRanks);
            }
        }

        $toPlace = $toPlace->sort(function ($a, $b) use ($lectureForPractice, $packageKeyForCard, $packageRanks, $baseOrder) {
            $linkedA = $a->training_type === 'lecture' || $lectureForPractice($a);
            $linkedB = $b->training_type === 'lecture' || $lectureForPractice($b);
            $rankA = $linkedA ? ($packageRanks[$packageKeyForCard($a)] ?? PHP_INT_MAX - 1) : PHP_INT_MAX;
            $rankB = $linkedB ? ($packageRanks[$packageKeyForCard($b)] ?? PHP_INT_MAX - 1) : PHP_INT_MAX;
            $typeA = $a->training_type === 'lecture' ? 0 : ($linkedA ? 1 : 2);
            $typeB = $b->training_type === 'lecture' ? 0 : ($linkedB ? 1 : 2);
            return [$rankA, $typeA, ...$baseOrder($a)] <=> [$rankB, $typeB, ...$baseOrder($b)];
        })->values();
        ...
        // Ma'ruza va unga bog'langan barcha guruh amaliylarini oldindan bitta
        // paket sifatida tekshiramiz. Ma'ruza alohida qo'yilib, amaliylar uchun
        // kun oxirida joy qolmay qolishi yoki boshqa fan oradagi slotni egallashi
        // mumkin emas: paketning hammasi sig'sagina barchasi birga joylashtiriladi.
        $packageHandledIds = [];
        if ($scopeType === null) {
            $toPlaceById = $toPlace->keyBy(fn($card) => (int) $card->id);
            $practicesByLecture = [];
            foreach ($toPlace as $candidate) {
                if ($candidate->training_type !== 'practice') {
                    continue;
                }
                $lecture = $lectureForPractice($candidate);
                if (!$lecture || $lecture->day || $lecture->pair || !$toPlaceById->has((int) $lecture->id)) {
                    continue;
                }
            ...
            foreach ($orderedLectures as $lecture) {
                ...
                foreach ($practiceGroups as $groupCards) {
                    ...
                    foreach ($groupCards as $position => $practice) {
                        ...
                        $practiceMask = $maskOf($practice);
                        $standIn = $position === 0 && (($practiceMask & $lectureMask) === 0);
                        $relative = $standIn
                        ...
                        $packageCards[] = $practice;
                    }
                }

                foreach ($packageCards as $packageCard) {
                    $packageHandledIds[(int) $packageCard->id] = true;
                }
                ...
                foreach ($descriptors as &$descriptor) {
                    ...
                    $pool = $poolFor($itemCard);
                    $descriptor['room_required'] = $pool->isNotEmpty();
                    $descriptor['pool'] = $descriptor['room_required']
                ...
                // Xonasi eng cheklangan (mos xona soni kam) kartalar avval xona
                // tanlasin. Aks holda erkin karta yagona mos xonani egallab,
                // aslida yechimi bor paket noto'g'ri rad etilishi mumkin.
                $fitDescriptors = $descriptors;
                usort($fitDescriptors, function (array $left, array $right) {
                    $leftRooms = $left['room_required'] ? count($left['pool']) : PHP_INT_MAX;
                    $rightRooms = $right['room_required'] ? count($right['pool']) : PHP_INT_MAX;
                    return [
                        $leftRooms,
                        -(int) $left['card']->students,
                        (int) $left['relative'],
                        (int) $left['card']->id,
                    ] <=> [
                        $rightRooms,
                        -(int) $right['card']->students,
                        (int) $right['relative'],
                        (int) $right['card']->id,
                    ];
                });
                    ...
                    for ($day = 1; $day <= $packageDays; $day++) {
                        for ($start = 1; $start + $lectureLen - 1 <= $packagePairs; $start++) {
                            ...
                            foreach ($fitDescriptors as $descriptor) {
                                ...
                                $chosenRoom = null;
                                if ($descriptor['room_required']) {
                                    foreach ($descriptor['pool'] as $room) {
                                        $roomFits = true;
                                        for ($offset = 0; $offset < (int) $descriptor['len']; $offset++) {
                                            $roomKey = $room->code . '|' . $day . '|' . ($itemPair + $offset);
                                            if (((($roomBusy[$roomKey] ?? 0) | ($internalRoomBusy[$roomKey] ?? 0)) & $itemMask) !== 0) {
                                                $roomFits = false;
                                                break;
                                            }
                                        }
                                ...
                                for ($offset = 0; $offset < (int) $descriptor['len']; $offset++) {
                                    ...
                                    $groupKey = $itemScope . '|' . $day . '|' . $slot;
                                    foreach ($itemGroups as $group) {
                                        $internalGroupBusy[$groupKey][$group] =
                                            ($internalGroupBusy[$groupKey][$group] ?? 0) | $itemMask;
                                    }
                                    if ($itemCard->teacher_id) {
                                        $teacherKey = $itemCard->teacher_id . '|' . $day . '|' . $slot;
                                        $internalTeacherBusy[$teacherKey] =
                                            ($internalTeacherBusy[$teacherKey] ?? 0) | $itemMask;
                                    }
                                    if ($chosenRoom) {
                                        $roomKey = $chosenRoom->code . '|' . $day . '|' . $slot;
                                        $internalRoomBusy[$roomKey] =
                                            ($internalRoomBusy[$roomKey] ?? 0) | $itemMask;
                                    }
                                }
                                $items[] = [
                                    'card' => $itemCard,
                ...
                foreach ($bestPackage as $item) {
                    /** @var TimetableCard $itemCard */
                    $itemCard = $item['card'];
                    $itemCard->day = $item['day'];
                    $itemCard->pair = $item['pair'];
                    $itemCard->start_half = 0;
                    if ($item['room']) {
                        $itemCard->auditorium_code = $item['room']->code;
                        $itemCard->auditorium_name = $item['room']->name;
                        $roomsAssigned++;
                    }
                    $this->markBusy($groupBusy, $teacherBusy, $roomBusy, $itemCard, $maskOf($itemCard));
                    $spread = $this->spreadKey($itemCard);
                    $subjDay[$spread . '|' . $itemCard->day] = ($subjDay[$spread . '|' . $itemCard->day] ?? 0) + 1;
                    $subjSlots[$spread][] = [
                        (int) $itemCard->day,
                        (int) $itemCard->pair,
                        $this->parasNeeded($itemCard),
                    ];
                    if ($clusterMode) {
                        $anchors[$anchorKey($itemCard)][(int) $itemCard->day][] = [
                            (int) $itemCard->pair,
                            (int) $itemCard->pair + $this->parasNeeded($itemCard) - 1,
                        ];
                    }
                    $touched[] = $itemCard;
                    $placed++;
                }
                $chain[$subjOf($lecture)] = [
                    'day' => (int) $lecture->day,
                    'lec' => (int) $lecture->pair,
                    'llen' => $lectureLen,
                    'next' => (int) $lecture->pair + $lectureLen,
                    'lw' => (int) $lecture->weeks,
                ];
            }

            if ($packageHandledIds) {
                $toPlace = $toPlace->reject(
                    fn($card) => isset($packageHandledIds[(int) $card->id])
                )->values();
            }
        }
        ...
        // Birinchi guruhning amaliyoti keyingi guruhni ma'ruzadan uzoqlashtirmasin.
        $practiceNext = [];
        foreach ($units as $unit) {
            ...
            if (count($unit) > 1) {
                ...
                if ($linkedToLecture) {
                    $uCh = $chain[$uChainKey];
                    $uPracticeKey = $uChainKey . '|' . (string) $lead->group_name;
                    $uCh['next'] = $practiceNext[$uPracticeKey] ?? $uCh['next'];
                    ...
                    if ($spot !== null) {
                        ...
                        $practiceNext[$uPracticeKey] = max((int) $uCh['next'], (int) $spot[0]['pair'] + $blockLen);
                    }
                }
            ...
            if ($c->training_type === 'practice' && $ch) {
                ...
                // Bog'langan amaliy boshqa kunni qidirmaydi.
                $unplaced++;
                continue;
            }
    ...
    /** Joylashmagan kartalar va ularning sabablarini tahliliy Excelga chiqarish. */
    public function unplacedDiagnosticsExport(Request $request, TimetableBoard $board)
    {
        $data = $request->validate([
            'faculty_names' => 'nullable|array',
            'faculty_names.*' => 'nullable|string|max:255',
        ...
        $query = TimetableCard::where('board_id', $board->id)
            ->whereNull('day')->whereNull('pair');
        $this->applyScopeToQuery($query, $facSet, $specSet, $courseSet);
        if (!empty($data['training_type'])) {
            $query->where('training_type', $data['training_type']);
        }

        $cycleKeys = [];
        if (Schema::hasTable('timetable_subject_settings')) {
            foreach (TimetableSubjectSetting::where('board_id', $board->id)->where('mode', 'cycle')->get() as $setting) {
                $cycleKeys[mb_strtolower(trim((string) $setting->specialty_name)) . '|'
                    . (int) $setting->course . '|'
                    . mb_strtolower(trim((string) $setting->subject_name))] = true;
            }
        }

        $unplaced = $query->get()->reject(fn(TimetableCard $card) => isset($cycleKeys[
            mb_strtolower(trim((string) $card->specialty_name)) . '|'
            . (int) $card->course . '|'
            . mb_strtolower(trim((string) $card->subject_name))
        ]))->sortBy(fn(TimetableCard $card) => implode('|', [
            $card->faculty_name, $card->specialty_name, str_pad((string) $card->course, 2, '0', STR_PAD_LEFT),
            $this->normSubject((string) $card->subject_name), $card->oqim_label, $card->group_name,
        ]))->values();

        $all = TimetableCard::where('board_id', $board->id)->get();
        $gridSettings = TimetableGridSetting::where('board_id', $board->id)->get()
            ->keyBy(fn($grid) => ($grid->faculty_name ?? '') . '|' . $grid->specialty_name . '|' . $grid->course);
        $gridFor = function (TimetableCard $card) use ($gridSettings, $board) {
            return $gridSettings[($card->faculty_name ?? '') . '|' . $card->specialty_name . '|' . $card->course]
                ?? $gridSettings['|' . $card->specialty_name . '|' . $card->course]
                ?? null;
        };
        ...
        $maskCache = [];
        $weeksCache = [];
        $maskOf = function (TimetableCard $card) use (&$maskCache, &$weeksCache, $cancelled, $gridFor, $board): int {
            $id = (int) $card->id;
            if (isset($maskCache[$id])) {
                return $maskCache[$id];
            }
            $grid = $gridFor($card);
            $total = max(1, min(60, (int) ($grid->weeks ?? $board->weeks)));
            $mask = 0;
            $active = [];
            for ($week = 1; $week <= $total; $week++) {
                if (!isset($cancelled[$id][$week])) {
                    $mask |= 1 << ($week - 1);
                    $active[] = $week;
                }
            }
            $weeksCache[$id] = [$total, $active];
            return $maskCache[$id] = ($mask ?: ((1 << $total) - 1));
        };

        $groupBusy = [];
        $teacherBusy = [];
        $roomBusy = [];
        foreach ($all as $placed) {
            if ($placed->day && $placed->pair) {
                $this->markBusy($groupBusy, $teacherBusy, $roomBusy, $placed, $maskOf($placed));
            }
        }
        ...
        $assignRooms = (bool) ($data['assign_rooms'] ?? false);
        $lectureRooms = (bool) ($data['lecture_rooms'] ?? false);
        $allRooms = ($assignRooms || $lectureRooms)
            ? Auditorium::where('active', true)->orderBy('volume')->get(['id', 'code', 'name', 'volume', 'auditorium_type_name'])
            : collect();
        $lecturePool = $allRooms->filter(fn($room) => mb_stripos((string) ($room->auditorium_type_name ?? ''), 'ruza') !== false)->values();
        if ($lecturePool->isEmpty()) {
            $lecturePool = $allRooms;
        }
        $roomTeacherMap = $this->auditoriumTeacherMap();
        $settings = $board->settings ?? [];
        $roomTolerance = max(0, min(30, (int) ($settings['room_tolerance_pct'] ?? 5)));
        $clusterMode = (bool) ($settings['pair_same_day'] ?? false) || (bool) ($settings['pair_consecutive'] ?? false);
        $dayNames = $board->day_names ?: TimetableBoard::DEFAULT_DAY_NAMES;

        $rows = [];
        foreach ($unplaced as $index => $card) {
            $mask = $maskOf($card);
            [$totalWeeks, $activeWeeks] = $weeksCache[(int) $card->id];
            $grid = $gridFor($card);
            $days = max(1, (int) ($grid->days ?? $board->days));
            $pairs = max(1, (int) $board->pairCount());
            $need = $this->parasNeeded($card);
            $groups = $card->occupiedGroups();
            $scope = $this->groupScopeKey($card);
            $minimumVolume = (int) ceil((int) $card->students * (100 - $roomTolerance) / 100);

            $pool = ($lectureRooms && $card->training_type === 'lecture')
                ? $lecturePool
                : ($assignRooms ? $allRooms : collect());
            $roomRequired = $pool->isNotEmpty();
            if ($roomRequired) {
                $pool = $pool->filter(fn($room) =>
                    (int) ($room->volume ?? 0) >= $minimumVolume
                    && $this->auditoriumAllowedForCard($room, $card, $roomTeacherMap)
                )->values();
            }
            ...
            if ($need > $pairs) {
                $reasonCode = 'lesson_too_long';
                $reason = "Dars uzunligi kunlik panjaraga sig'maydi.";
                $recommendation = "Dars uzunligi yoki kunlik paralar sonini tekshiring.";
            } elseif ($roomRequired && $pool->isEmpty()) {
                $reasonCode = 'room_capacity';
                $reason = "Talabalar soniga mos yoki o'qituvchiga ruxsat etilgan auditoriya yo'q.";
                $recommendation = "Kamida {$minimumVolume} o'rinli faol xona va xona-o'qituvchi biriktirishini tekshiring.";
            } else {
                $slotReasons = function (int $day, int $pair) use (
                    $card, $mask, $need, $groups, $scope, $groupBusy, $teacherBusy,
                    $roomBusy, $roomRequired, $pool
                ): array {
                    $blocked = ['group_busy' => false, 'teacher_busy' => false, 'room_busy' => false];
                    for ($offset = 0; $offset < $need; $offset++) {
                        $slot = $pair + $offset;
                        $groupKey = $scope . '|' . $day . '|' . $slot;
                        foreach ($groups as $group) {
                            if ((($groupBusy[$groupKey][$group] ?? 0) & $mask) !== 0) {
                                $blocked['group_busy'] = true;
                                break;
                            }
                        }
                        if ($card->teacher_id && ((($teacherBusy[$card->teacher_id . '|' . $day . '|' . $slot] ?? 0) & $mask) !== 0)) {
                            $blocked['teacher_busy'] = true;
                        }
                    }
                    if ($roomRequired) {
                        $hasFreeRoom = false;
                        foreach ($pool as $room) {
                            $roomFree = true;
                            for ($offset = 0; $offset < $need; $offset++) {
                                if ((($roomBusy[$room->code . '|' . $day . '|' . ($pair + $offset)] ?? 0) & $mask) !== 0) {
                                    $roomFree = false;
                                    break;
                                }
                            }
                        ...
                        $blocked['room_busy'] = !$hasFreeRoom;
                    }
                    return array_keys(array_filter($blocked));
                };
                $checkSlot = function (int $day, int $pair) use (&$checked, &$free, &$stats, &$examples, $slotReasons, $dayNames) {
                    $checked++;
                    $blocked = $slotReasons($day, $pair);
                    if (!$blocked) {
                        $free++;
                        return;
                    }
                    ...
                    if (count($examples) < 5) {
                        $labels = ['group_busy' => 'guruh', 'teacher_busy' => "o'qituvchi", 'room_busy' => 'auditoriya'];
                        $examples[] = ($dayNames[$day - 1] ?? $day . '-kun') . ', ' . $pair . '-slot: '
                            . implode(', ', array_map(fn($code) => $labels[$code], $blocked));
                    }
                };

                $chainLecture = $card->training_type === 'practice'
                    ? $this->matchingLectureForPractice($card, $lecturesByBase)
                    : null;
                if ($chainLecture && (!$chainLecture->day || !$chainLecture->pair)) {
                    $reasonCode = 'lecture_unplaced';
                    $reason = "Mos ma'ruza hali joylashtirilmagan.";
                    $recommendation = "Avval {$chainLecture->subject_name} ma'ruzasini joylashtiring.";
                } elseif ($chainLecture) {
                    $standIn = (int) $card->weeks === $totalWeeks - (int) $chainLecture->weeks;
                    $expectedPair = $standIn
                        ? (int) $chainLecture->pair + max(0, $this->parasNeeded($chainLecture) - $need)
                        : (int) $chainLecture->pair + $this->parasNeeded($chainLecture);
                    if ($expectedPair < 1 || $expectedPair + $need - 1 > $pairs) {
                        $reasonCode = 'chain_out_of_grid';
                        $reason = "Ma'ruzadan keyin amaliy uchun yetarli para qolmagan.";
                        $recommendation = "Ma'ruzani ertaroq paraga ko'chiring yoki panjarani kengaytiring.";
                    } else {
                        $checkSlot((int) $chainLecture->day, $expectedPair);
                        if ($free) {
                            $reasonCode = 'chain_order';
                            $reason = "Ma'ruzaga bog'langan slot hozir bo'sh, ammo joylash navbatida fan zanjiri sig'magan.";
                            $recommendation = "Shu fan qamrovini to'liq qaytadan joylashtiring.";
                        } else {
                            $reasonCode = 'chain_slot_blocked';
                            $reason = "Amaliy uchun ma'ruzadan keyingi majburiy slot band.";
                            $recommendation = "Ma'ruzadan keyingi guruh/o'qituvchi/xona to'qnashuvini bo'shating.";
                        }
                    }
                } else {
                    for ($day = 1; $day <= $days; $day++) {
                    ...
                    if ($free > 0 && $clusterMode) {
                        $reasonCode = 'subject_block_did_not_fit';
                        $reason = "Karta uchun bo'sh slot bor, ammo fan-guruhning yaxlit bloki birga sig'magan.";
                        $recommendation = "Uzluksiz joy bo'shating yoki bir kunga/ketma-ket qoidasini yumshating.";
                    } elseif ($free > 0) {
                        $reasonCode = 'free_slot_exists';
                        $reason = "Hozir mos bo'sh slot bor, karta oldingi joylash navbatida o'tkazib yuborilgan.";
                        $recommendation = "Tanlangan qamrovni qaytadan joylashtiring.";
                    } else {
                        arsort($stats);
                        $topCode = array_key_first($stats);
                        $reasonCode = $topCode ?: 'no_slot';
                        $reasonLabels = [
                            'group_busy' => 'Guruh bandligi asosiy to\'siq.',
                            'teacher_busy' => "O'qituvchi bandligi asosiy to'siq.",
                            'room_busy' => 'Auditoriya bandligi asosiy to\'siq.',
                        ];
                        $reason = $reasonLabels[$reasonCode] ?? "Mos bo'sh slot topilmadi.";
                        $recommendation = "Bandlikni bo'shating yoki kun/paralar sonini oshiring.";
                    }
                }
            }

            $weekText = count($activeWeeks) === $totalWeeks
                ? 'Barcha (1-' . $totalWeeks . ')'
                : implode(', ', $activeWeeks);
            $groupText = $card->training_type === 'lecture'
                ? implode(', ', $card->group_names ?: [])
                : (string) $card->group_name;
            $rows[] = [
                $index + 1,
                (int) $card->id,
        ...
        $safeBoard = preg_replace('/[^\pL\pN_-]+/u', '_', (string) $board->name) ?: 'dars_jadvali';
        $filename = $safeBoard . '_joylashmagan_kartalar_' . now()->format('Ymd_His') . '.xlsx';

        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\TimetableUnplacedDiagnosticsExport($rows),
            $filename
        );
    }
    ...
    private function placementBaseKey(TimetableCard $card): string
    {
    ...
    private function placementSubjectKey(TimetableCard $card): string
    {
        return $this->placementBaseKey($card) . '|' . (string) $card->oqim_label;
    }

    /** @param array<string,array<int,TimetableCard>> $lecturesByBase */
    private function matchingLectureForPractice(TimetableCard $practice, array $lecturesByBase): ?TimetableCard
    {
        $candidates = $lecturesByBase[$this->placementBaseKey($practice)] ?? [];
        if (!$candidates) {
            return null;
        }

        $group = trim((string) $practice->group_name);
        if ($group !== '') {
            $matches = array_values(array_filter(
                $candidates,
                fn(TimetableCard $lecture) => in_array($group, $lecture->occupiedGroups(), true)
            ));
            if ($matches) {
                $candidates = $matches;
            }
        ...
        $matches = array_values(array_filter(
            $candidates,
            fn(TimetableCard $lecture) => (string) $lecture->oqim_label === (string) $practice->oqim_label
        ));
        if ($matches) {
            $candidates = $matches;
        }

        usort($candidates, fn(TimetableCard $a, TimetableCard $b) => (int) $a->id <=> (int) $b->id);
        return $candidates[0] ?? null;
    }

    /** Fan taqsimoti: ma'ruza oqim, amaliyot guruhcha bo'yicha klasterlanadi. */
