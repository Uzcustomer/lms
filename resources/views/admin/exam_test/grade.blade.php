            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 pt-2 bg-white border-b border-gary-200">
                    ...
                    <div class="flex w-full">
                        <div class="w-3/4">
                            <form method="POST" class="inline" action="{{ route('admin.examtest.sababli.save') }}"
                                enctype="multipart/form-data">

                                @csrf
                                <input type="hidden" value="{{$examtest->id}}" name="examtest">
                                    ...
                                    @foreach ($students as $student)
                                        <div class="px-2 py-2 row sm:px-0">
                                            ...
                                            @if (empty($student->grade) and $student->grade != 0)
                                                @if($examtest->status == 1 and auth()->user()->hasRole(['admin']))
                                                    <?php 
                                                                                                                                                                                                                                                                                                                                                                                                                                $check = in_array($student->hemis_id, json_decode($examtest->sababli_studets) ?? []);
                                                                                                                                                                                                                                                                                                                                                                                                                            ?>
                                                    @if(!$check)
                                                    ...
                                                    @else
                                                        ...
                                                        <dd class="mt-1 text-sm  font-medium text-red-500 col-2">
                                                            <a href="{{ route('admin.examtest.file.delete', $student->exam_file_id) }}"
                                ...
                                @if ($examtest->status == 1 && auth()->user()->hasRole(['admin']))
                                ...
                                @endif
                                @if ($examtest->status == 0 && auth()->user()->hasRole(['examiner']))
                                    <div class="mt-6">
                                        <a type="submit" href="{{ route('admin.examtest.grade_form', $examtest->id) }}"
                                            class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                                            Baholash
                                        </a>
                                    </div>
