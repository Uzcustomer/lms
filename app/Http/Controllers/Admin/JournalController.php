...
class JournalController extends Controller
{
    ...
    /**
     * YN ga yuborishni bekor qilish (faqat admin/superadmin)
     */
    public function undoYnSubmission(Request $request)
    {
        if (!auth()->user()?->hasAnyRole(['superadmin', 'admin'])) {
            return response()->json([
                'success' => false,
                'message' => 'Bu amalni faqat admin bajara oladi.',
            ], 403);
        }
        ...
        if (!$ynSubmission) {
            return response()->json([
                'success' => false,
                'message' => 'YN yuborilmagan.',
            ], 404);
        }

        DB::beginTransaction();
        try {
            // Talabalar hemis_id larini olish
            $studentHemisIds = YnStudentGrade::where('yn_submission_id', $ynSubmission->id)
                ->pluck('student_hemis_id')
                ->toArray();

            // YN student grades o'chirish
            YnStudentGrade::where('yn_submission_id', $ynSubmission->id)->delete();

            // student_grades qulfini ochish
            ...
            return response()->json([
                'success' => true,
                'message' => 'YN ga yuborish bekor qilindi. Baholar qulfi ochildi.',
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json([
