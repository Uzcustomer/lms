<x-student-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-[14px] text-gray-800 leading-tight">
            {{ __('Talaba profili') }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 bg-white border-b border-gray-200">
                    <div class="flex flex-col md:flex-row">
                        ...
                        <div class="md:w-3/4 mt-3 md:mt-0">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="bg-gray-50 p-4 rounded-lg">
                                    <h4 class="font-semibold text-[14px] mb-2 text-gray-700">{{ __("Shaxsiy ma'lumotlar") }}</h4>
                                    ...
                                    <div class="mt-3 pt-2 border-t border-gray-200">
                                        @if(session('success'))
                                            <div class="mb-2 px-3 py-2 bg-green-100 border border-green-300 text-green-700 rounded-lg text-[14px] font-medium">{{ session('success') }}</div>
                                        @endif
                                        @if($errors->any())
                                            <div class="mb-2 px-3 py-2 bg-red-100 border border-red-300 text-red-700 rounded-lg text-[14px] font-medium">
                                        ...
                                        <form method="POST" action="{{ route('student.profile.update-contact') }}">
                                            ...
                                            <div class="space-y-2">
                                                <div>
                                                    <label class="text-[14px] font-medium text-gray-600">{{ __('Telefon raqam') }}</label>
                                                    <input type="text" name="phone" value="{{ old('phone', $profileData['phone']) }}"
                                                           placeholder="+998901234567"
                                                           class="mt-1 w-full px-3 py-2 text-[14px] border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                                                </div>
                                                <div>
                                                    <label class="text-[14px] font-medium text-gray-600">{{ __('Telegram username') }}</label>
                                                    <input type="text" name="telegram_username" value="{{ old('telegram_username', $profileData['telegram_username']) }}"
                                                           placeholder="@username"
                                                           class="mt-1 w-full px-3 py-2 text-[14px] border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                                                </div>
                                                <button type="submit"
                                                        class="w-full px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white text-[14px] font-semibold rounded-lg transition">
                                                    {{ __('Saqlash') }}
                                                </button>
                                            </div>
                                        </form>
                                        ...
                                        @if($student->telegram_username && $student->telegram_verification_code && !$student->telegram_verified_at)
                                            <div class="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                                <p class="text-[14px] font-semibold text-blue-800 mb-2">{{ __('Telegram tasdiqlash') }}</p>
                                                <p class="text-[14px] text-gray-500 mb-1">{{ __('Username:') }} <strong>{{ $student->telegram_username }}</strong></p>
                                                <p class="text-[14px] text-gray-600 mb-1">{{ __('Tasdiqlash kodingiz:') }}</p>
                                                ...
                                                @if($botUsername)
                                                    <a href="https://t.me/{{ $botUsername }}?start={{ $student->telegram_verification_code }}"
                                                       target="_blank"
                                                       class="w-full inline-flex justify-center items-center px-3 py-2 bg-[#0088cc] text-white text-[14px] font-medium rounded-lg hover:bg-[#007ab8] transition">
                                                ...
                                                @endif
                                                <div id="profile-tg-status" class="hidden mt-2 p-2 bg-green-50 border border-green-200 rounded-lg text-[14px] text-green-700 font-medium text-center">
                                        ...
                                        @elseif($student->telegram_verified_at)
                                            <div class="mt-2 p-3 bg-green-50 border border-green-200 rounded-lg flex items-center gap-2">
                                                ...
                                                <span class="text-[14px] font-medium text-green-700">{{ __('Telegram tasdiqlangan') }}: {{ $student->telegram_username }}</span>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                                <div class="bg-gray-50 p-4 rounded-lg">
                                    <h4 class="font-semibold text-[14px] mb-2 text-gray-700">{{ __("Ta'lim ma'lumotlari") }}</h4>
                                    <ul class="space-y-1.5 text-[14px]">
                                        ...
                                        <li>
                                            ...
                                            @if($profileData['is_graduate'])
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[14px] font-medium bg-green-100 text-green-800">{{ __('Bitiruvchi') }}</span>
                                            @else
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[14px] font-medium bg-blue-100 text-blue-800">{{ __('Talaba') }}</span>
                                            @endif
                                        </li>
                                    </ul>
                                </div>
                            ...
                            <div class="mt-4 bg-gray-50 p-4 rounded-lg">
                                <h4 class="font-semibold text-[14px] mb-2 text-gray-700">{{ __("Qabul ma'lumotlari") }}</h4>
                                <ul class="space-y-1.5 text-[14px]">
                            ...
                            <div class="mt-4 bg-gray-50 p-4 rounded-lg">
                                <h4 class="font-semibold text-[14px] mb-2 text-gray-700">{{ __('Manzil') }}</h4>
                                <ul class="space-y-1.5 text-[14px]">
                    ...
                    <div class="mt-4 pt-3 border-t border-gray-200">
                        <form method="POST" action="{{ route('student.logout') }}">
                            @csrf
                            <button type="submit"
                                    class="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-red-50 hover:bg-red-100 text-red-600 text-[14px] font-semibold rounded-lg transition">
