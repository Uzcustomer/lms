<x-teacher-app-layout>
    ...
    <div class="tutor-container" x-data="{
            ...
            <div class="group-grid">
                @foreach($tutorGroups as $group)
                    ...
                    <a href="{{ route('teacher.students', ['group' => $group->group_hemis_id]) }}"
                       class="group-card"
                       @click.prevent="selectedGroup = '{{ $group->group_hemis_id }}'; window.history.pushState({}, '', '?group={{ $group->group_hemis_id }}');">
        ...
        {{-- Talabalar ro'yxati --}}
        <div x-show="selectedGroup" x-cloak>
            <a href="{{ route('teacher.students') }}" class="back-btn" @click.prevent="selectedGroup = ''; window.history.pushState({}, '', '{{ route('teacher.students') }}');">
                <svg style="width:16px;height:16px;" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5"/></svg>
                Guruhlar
            </a>
