...
class JournalController extends Controller
{
    ...
    private function showJournal(Request $request, $groupId, $subjectId, $semesterCode)
    {
        ...
        // Kengaytirilgan sababli arizalar ro'yxati — assessment_type yoki makeup
        // chegarasisiz. Ma'ruza tab va Dav % hisobi shu yerdan sababli oraliqlarni
        // o'qiydi. Bitta talabaning bir necha arizasi bo'lishi mumkin → groupBy.
        $broadExcuses = collect();
        try {
            $broadExcuses = AbsenceExcuse::where('status', 'approved')
                ->whereIn('student_hemis_id', $studentHemisIds)
                ->get()
                ->groupBy('student_hemis_id');
        } catch (\Exception $e) {
            Log::warning('AbsenceExcuse broad query failed: ' . $e->getMessage());
        }
    ...
    /**
     * computeJnAveragesForGroup'ning MT (training_type_code=99) versiyasi —
     * Sinov fanlar Excel eksporti uchun jurnal sahifasi bilan bir xil formulani
     * ishlatadi (kunlik gradeSum / pairsInDay → HALF_UP, sum / totalDays → HALF_UP).
     */
    private function computeMtAveragesForGroup(string $subjectId, string $semesterCode, string $groupHemisId): array
    {
        ...
        $gradingCutoffDate = \Carbon\Carbon::now('Asia/Tashkent')->endOfDay();
        $mtLessonDatesForAverage = array_values(array_filter($mtLessonDates, function ($date) use ($gradingCutoffDate) {
            return \Carbon\Carbon::parse($date, 'Asia/Tashkent')->startOfDay()->lte($gradingCutoffDate);
        }));
        $totalMtDaysForAverage = count($mtLessonDatesForAverage);

        if ($totalMtDaysForAverage === 0) {
            return array_fill_keys($studentHemisIds, 0);
        }
        ...
        foreach ($studentHemisIds as $hemisId) {
            $dailySum = 0;
            $studentDayGrades = $mtGrades[$hemisId] ?? [];
            foreach ($mtLessonDatesForAverage as $date) {
                $dayGrades = $studentDayGrades[$date] ?? [];
            ...
            $result[$hemisId] = $totalMtDaysForAverage > 0
                ? (int) round($dailySum / $totalMtDaysForAverage, 0, PHP_ROUND_HALF_UP)
                : 0;
        }
    ...
    /**
     * YN qaydnoma Excel eksport — jurnal sahifasidan bitta guruh + fan uchun.
     * D20 - JN, G20 - MT, J20 - ON, P20 - OSKI, S20 - Test
     */
