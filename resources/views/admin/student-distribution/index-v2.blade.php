<x-app-layout>
    <style>
        ...
        .sd-accordion[open]{border-left:3px solid #38bdf8;background:#f0f9ff;box-shadow:inset 0 0 0 1px #bae6fd}
        .sd-accordion[open]>summary{background:linear-gradient(90deg,#e0f2fe,#ecfeff);color:#0c4a6e}
        .sd-accordion[open]>.sd-accordion-body{background:#f8fdff}
    ...
    <script>
        (() => {
            ...
            const isD12Group = name => /^d[12](?:\/|$)/i.test(String(name || '').trim());
            const compatibleFaculty = (sourceFaculty, sourceGroup, targetFaculty, targetGroup) =>
                sourceFaculty === targetFaculty || (isD12Group(sourceGroup) && isD12Group(targetGroup));
            ...
            //   d1/23-09b (2 talik guruh)  -> d1/23-09b   (izoh qavs tashlab yuboriladi)
            //
            // Oxiridagi qavs ikki xil bo'ladi: "(a)", "(b)", "(rus)" — guruhni
            // ajratadigan qism, saqlanadi; "(2 talik guruh)" kabi ko'p so'zli
            // izoh esa nomga taalluqli emas, olib tashlanadi.
            ...
            async function loadManual() {
                ...
                const filtered = groups.filter(group => group.is_source
                    && (!f.faculty || compatibleFaculty(group.faculty_name, group.group_name, f.faculty, selectedGroup?.group_name))
                    && (!f.specialty || group.specialty_name === f.specialty)
                    && (!f.course || String(group.course) === f.course));
                ...
                filtered.forEach(async group => {
                    const groupScope = query({faculty:group.faculty_name,specialty:group.specialty_name,course:group.course,group_id:group.id});
                    const data = await fetch(urls.students+'?'+groupScope).then(response => response.json());
            ...
            async function openMove(id) {
                ...
                const available = groups.filter(group => !group.is_source && group.free_places > 0 && student
                    && compatibleFaculty(student.faculty, student.group_name, group.faculty_name, group.group_name)
                    && group.specialty_name === student.specialty && Number(group.course) === Number(student.course));
