...
<aside x-data="sidebarTheme()" :data-theme="theme"
    ...
    <nav class="flex-1 py-3 px-3 overflow-y-auto sidebar-nav"
        ...
        @if($hasActiveRole(['superadmin', 'admin', 'kichik_admin', 'registrator_ofisi', 'oquv_bolimi', 'oquv_prorektori']))
    ...
    <div class="p-3 flex-shrink-0 sidebar-user-section" x-data="{ profileOpen: false, rolesOpen: false }" @click.outside="profileOpen = false; rolesOpen = false">
        ...
        <div x-show="profileOpen"
            ...
            <div class="py-1 sidebar-dropdown-divider-bottom">
                ...
                @if($hasActiveRole(['superadmin', 'admin']))
                <form method="POST" action="{{ route('admin.international-students.toggle-false-show') }}" class="px-3 py-2">
                    @csrf
                    @php $fse = \App\Models\Setting::get('false_show_enabled', '0') === '1'; @endphp
                    <button type="submit" class="profile-dropdown-link w-full text-left" style="display:flex;align-items:center;gap:8px;padding:6px 0;">
                        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                        <span style="flex:1;">False Show</span>
                        <span style="font-size:10px;font-weight:700;padding:2px 8px;border-radius:10px;{{ $fse ? 'background:#dcfce7;color:#166534;' : 'background:#f1f5f9;color:#94a3b8;' }}">{{ $fse ? 'ON' : 'OFF' }}</span>
                    </button>
                </form>
                @endif
            </div>
