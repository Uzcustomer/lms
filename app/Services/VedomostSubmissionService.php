...
class VedomostSubmissionService
{
    ...
    /**
     * Admin qo'lda ochgan shaklni yaratadi yoki mavjud bo'lsa yangilamasdan qaytaradi.
     */
    public function manualOpen(VedomostSubmission $source, string $formType): VedomostSubmission
    {
        $allowedForms = VedomostSubmission::manualOpenableForms($source->closing_form);
        if (!isset($allowedForms[$formType])) {
            throw \Illuminate\Validation\ValidationException::withMessages([
                'form_type' => "Bu yopilish shakli uchun " . VedomostSubmission::formLabel($formType) . " ni qo'lda ochib bo'lmaydi.",
            ]);
        }

        return match ($formType) {
            VedomostSubmission::FORM_12,
            VedomostSubmission::FORM_12Q,
            VedomostSubmission::FORM_12AG,
            VedomostSubmission::FORM_12BG => $this->manualOpenGroupScopedForm($source, $formType),
            VedomostSubmission::FORM_12A,
            VedomostSubmission::FORM_12B,
            VedomostSubmission::FORM_12AQ,
            VedomostSubmission::FORM_12BQ => $this->manualOpenCombinedForm($source, $formType),
            default => throw \Illuminate\Validation\ValidationException::withMessages([
                'form_type' => "Qo'lda ochish uchun noma'lum shakl turi berildi.",
            ]),
        };
    }
    ...
    /**
     * Guruhga bog'liq shakllarni qo'lda ochadi: 12 / 12q / 12ag / 12bg.
     */
    private function manualOpenGroupScopedForm(VedomostSubmission $source, string $formType): VedomostSubmission
    {
        $base = $this->manualBaseGroupRow($source);
        if (!$base) {
            throw \Illuminate\Validation\ValidationException::withMessages([
                'source' => "Qo'lda ochish uchun shu guruhning asosiy 12-shakli topilmadi.",
            ]);
        }
    ...
    /**
     * Umumiy (bir nechta guruh bitta) shakllarni qo'lda ochadi.
     */
    private function manualOpenCombinedForm(VedomostSubmission $source, string $formType): VedomostSubmission
    {
        $baseRows = $this->manualUnitBaseRows($source);
        if ($baseRows->isEmpty()) {
            throw \Illuminate\Validation\ValidationException::withMessages([
                'source' => "Qo'lda ochish uchun ushbu birlikning asosiy 12-shakllari topilmadi.",
            ]);
        }

        $template = $baseRows->sortBy('group_hemis_id', SORT_NATURAL)->first();
        $groupIds = $baseRows->pluck('group_hemis_id')->filter()->unique()->sort()->values();
        $repGroupId = $groupIds->first();
        if (!$repGroupId) {
            throw \Illuminate\Validation\ValidationException::withMessages([
                'source' => "Umumiy shakl uchun guruh aniqlanmadi.",
            ]);
        }
        ...
        $keys = [
            ...
            'semester_code' => $template->semester_code,
            'form_type' => $formType,
        ];

        $baseDate = $this->manualBaseDateForForm($source, $formType, $template);
        $data = [
            'education_year' => $template->education_year,
            'group_name' => 'Barcha guruhlar (' . $groupIds->count() . ' ta)',
            'curriculum_hemis_id' => $template->curriculum_hemis_id,
            'curriculum_subject_id' => null,
            'subject_name' => $template->subject_name,
            'department_hemis_id' => $template->department_hemis_id,
            'department_name' => $template->department_name,
            'specialty_name' => $template->specialty_name,
            'closing_form' => $template->closing_form,
            'teacher_hemis_id' => null,
            'teacher_name' => null,
            'teacher_phone' => null,
            'fan_masuli_hemis_id' => $template->fan_masuli_hemis_id,
            'fan_masuli_name' => $template->fan_masuli_name,
            'fan_masuli_phone' => $template->fan_masuli_phone,
            'kafedra_mudiri_hemis_id' => $template->kafedra_mudiri_hemis_id,
            'kafedra_mudiri_name' => $template->kafedra_mudiri_name,
            'kafedra_mudiri_phone' => $template->kafedra_mudiri_phone,
            'base_type' => 'exam',
            'base_date' => $baseDate,
            'deadline' => $this->manualDeadlineFromBaseDate($baseDate),
        ];
    ...
    /**
     * Qo'lda ochishda tayanch bo'ladigan guruhning 12-shakli.
     */
    private function manualBaseGroupRow(VedomostSubmission $source): ?VedomostSubmission
    {
        if ($source->form_type === VedomostSubmission::FORM_12) {
            return $source;
        }

        return VedomostSubmission::query()
            ->where('group_hemis_id', $source->group_hemis_id)
            ->where('subject_id', $source->subject_id)
            ->where('semester_code', $source->semester_code)
    ...
    /**
     * Qo'lda ochishda bir birlikka tegishli barcha 12-shakl bazalarini qaytaradi.
     */
    private function manualUnitBaseRows(VedomostSubmission $source): Collection
    {
        $query = VedomostSubmission::query()
            ->where('subject_id', $source->subject_id)
            ->where('semester_code', $source->semester_code)
            ->where('curriculum_hemis_id', $source->curriculum_hemis_id)
            ->where('department_hemis_id', $source->department_hemis_id)
            ->where('specialty_name', $source->specialty_name)
            ->where('education_year', $source->education_year)
            ->where('form_type', VedomostSubmission::FORM_12);
        ...
        $base = $this->manualBaseGroupRow($source);
        return $base ? collect([$base]) : collect();
    }

    /**
     * Manual yaratishda oldingi bir xil shakldan sanani oladi, bo'lmasa manbadan foydalanadi.
     */
    private function manualBaseDateForForm(VedomostSubmission $source, string $formType, VedomostSubmission $template): ?string
    {
        $existing = VedomostSubmission::query()
            ->where('subject_id', $template->subject_id)
            ->where('semester_code', $template->semester_code)
            ->where('curriculum_hemis_id', $template->curriculum_hemis_id)
            ->where('department_hemis_id', $template->department_hemis_id)
            ->where('specialty_name', $template->specialty_name)
            ->where('education_year', $template->education_year)
            ->where('form_type', $formType)
            ->orderByDesc('id')
            ->first();

        if ($existing?->base_date) {
            return $existing->base_date->toDateString();
        }

        if ($source->base_date) {
            return $source->base_date->toDateString();
        }

        if ($template->base_date) {
            return $template->base_date->toDateString();
        }
