        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-3">
            ...
            <div class="bg-white shadow rounded-lg p-5 mb-4 border border-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                        <span class="block text-xs text-gray-500">{{ __('Telegram') }}</span>
                        <span class="font-medium text-gray-800">{{ $student->telegram_username ? '@' . $student->telegram_username : '-' }}</span>
                    </div>
                </div>
            </div>
            ...
            @if(!$visaInfo)
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
                    <svg class="w-12 h-12 text-yellow-400 mx-auto mb-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z"/>
                    </svg>
                    <p class="text-sm text-yellow-700 font-medium">{{ __('Talaba hali viza ma\'lumotlarini kiritmagan.') }}</p>
                </div>
            @else
                {{-- Holat va amallar --}}
                <div class="bg-white shadow rounded-lg p-5 mb-4 border border-gray-200" x-data="{ showRejectForm: false }">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex gap-2">
                            <form method="POST" action="{{ route('admin.international-students.approve', $student) }}">
                                @csrf
                    ...
                    {{-- Rad etish formasi --}}
                    <div x-show="showRejectForm" x-transition class="mt-3">
                        <form method="POST" action="{{ route('admin.international-students.reject', $student) }}">
                            @csrf
                            <div class="mb-3">
                                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Rad etish sababi') }} <span class="text-red-500">*</span></label>
                                <textarea name="rejection_reason" rows="3" required
                                          class="w-full text-sm rounded-lg border-gray-300 focus:ring-red-500 focus:border-red-500"
                                          placeholder="Sababni kiriting..."></textarea>
                            </div>
                            <button type="submit" class="px-4 py-2 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 transition">
                                {{ __('Rad etishni tasdiqlash') }}
                            </button>
                        </form>
                    </div>
                ...
                <div class="bg-white shadow rounded-lg p-5 mb-4 border border-gray-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <a href="{{ route('admin.international-students.file', [$student, 'passport_scan_path']) }}" target="_blank"
                        ...
                        <a href="{{ route('admin.international-students.file', [$student, 'visa_scan_path']) }}" target="_blank"
                           class="flex items-center gap-2 p-3 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition">
                            <svg class="w-5 h-5 text-red-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/>
                            </svg>
                        ...
                        <a href="{{ route('admin.international-students.file', [$student, 'registration_doc_path']) }}" target="_blank"
                           class="flex items-center gap-2 p-3 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition">
                            <svg class="w-5 h-5 text-red-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/>
                            </svg>
