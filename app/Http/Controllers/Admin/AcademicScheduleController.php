...
class AcademicScheduleController extends Controller
{
    ...
    public function tvJadval(Request $request)
    {
        ...
        // --- Rejalangan, lekin hali kompyuter taqsimlanmagan imtihonlar ---
        // TV displey "keyingi 60 daqiqa jadvali" bo'lgani uchun, hali
        // computer_assignments yaratilmagan imtihonlarni ham exam_schedules'dan
        // o'qib, guruh darajasidagi "rejada" kartochkasi sifatida ko'rsatamiz.
        // Kompyuter raqamlari taqsimlangach ular per-talaba kartochkalarga
        // almashadi.
        $assignedCombos = [];
        foreach (
            ...
            $assignedCombos[$r->exam_schedule_id . '|' . strtolower((string) $r->yn_type) . '|' . (int) $r->attempt] = true;
        }
        ...
        // Yaqinda o'tib ketgan (taqsimlanmagan) imtihonlar ham qisqa muddat
        // ko'rinib tursin — pastki chegara 20 daqiqa.
        $pendingWindowStart = $now->copy()->subMinutes(20);

        $plannedSchedules = ExamSchedule::with('group')
            ->where(function ($q) use ($date) {
                $q->whereDate('oski_date', $date)
                    ->orWhereDate('test_date', $date)
                    ->orWhereDate('oski_resit_date', $date)
                    ->orWhereDate('test_resit_date', $date)
                    ->orWhereDate('oski_resit2_date', $date)
                    ->orWhereDate('test_resit2_date', $date);
            })
            ->get();

        $seenPending = [];
        foreach ($plannedSchedules as $schedule) {
            foreach ($attemptDefs as [$yn, $attempt, $dateField, $timeField, $naField]) {
                $rawDate = $schedule->{$dateField} ?? null;
                $rawTime = $schedule->{$timeField} ?? null;
                if (!$rawDate || !$rawTime) {
                    continue;
                }
                if ($rawDate->format('Y-m-d') !== $date) {
                    continue;
                }
                if ($naField !== null && $schedule->{$naField}) {
                    continue;
                }
                // Bu (schedule, yn, attempt) uchun komp taqsimlangan bo'lsa —
                // u yuqorida per-talaba kartochka sifatida chiqqan.
                if (isset($assignedCombos[$schedule->id . '|' . $yn . '|' . $attempt])) {
                    continue;
                }

                $timeStr = \Carbon\Carbon::parse($rawTime)->format('H:i');
                $plannedStart = \Carbon\Carbon::parse($date . ' ' . $timeStr);
                if ($plannedStart->gt($windowEnd) || $plannedStart->lt($pendingWindowStart)) {
                    continue;
                }

                // Guruh-level va per-student qatorlar bir xil (guruh, fan, yn,
                // urinish, vaqt) bersa — bitta kartochka qoldiramiz.
                $dedupKey = ($schedule->group_hemis_id ?? '') . '|' . ($schedule->subject_id ?? '')
                    . '|' . $yn . '|' . $attempt . '|' . $timeStr;
                if (isset($seenPending[$dedupKey])) {
                    continue;
                }
                $seenPending[$dedupKey] = true;

                $items[] = [
                    'computer_number' => null,
                    'show_computer'   => false,
                    'short_name'      => (string) ($schedule->group?->name ?? $schedule->group_hemis_id ?? '—'),
                    'group_name'      => '',
                    'subject_name'    => (string) ($schedule->subject_name ?? ''),
                    'yn_type'         => strtoupper($yn),
                    'attempt'         => $attempt,
                    'planned_time'    => $timeStr,
                    'status'          => 'pending',
                    'minutes_until'   => max(0, (int) ceil($now->diffInSeconds($plannedStart, false) / 60)),
                ];
            }
        }
