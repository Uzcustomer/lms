    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('74 soat dars qoldirish') }}
            </h2>
            @if(auth()->user()->hasRole(['admin']))

                <a href="{{ route('admin.absence_report.create') }}" class="btn btn-primary">
                    {{ __('74 soat dars qoldirish qo\'shish') }}
                </a>
            @endif
    ...
    <div class="py-12">
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                ...
                <form id="search-form" method="GET" action="{{ route('admin.absence_report.index') }}" class="p-6 bg-gray-50">
                            ...
                            @else
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach ($absenceReports as $report)
                                            <tr>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                                    {{$report->deportment_name}}
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                                    {{$report->group_name}}
                                                </td>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                                    {{$report->user?->name ?? $report->dekan->full_name}}
                                                </td>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    @if(auth()->user()->hasRole(['admin']))
                                                        ...
                                                        <form action="{{ route('admin.absence_report.delete', $report->id) }}"
                                                            method="POST" class="inline"
                                                            onsubmit="return confirm('Haqiqatan ham bu yozuvni o\'chirmoqchimisiz?');">
                                                            @csrf
                                                            @method('DELETE')
